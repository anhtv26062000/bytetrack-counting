...
from yolox.utils import plot_tracking
...
def run(
        ...
        if seen % skip_frames == 0:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    ...
                    # Tracking - Bytetrack
                    online_targets, previous_targets = tracker.update(det[:, :-1], [img_info['height'], img_info['width']], [img_info['height'], img_info['width']])
                    online_tlwhs = []
                    online_ids = []
                    online_scores = []
                    ...
                    for pre_t in (previous_targets):
                        ...
                        if pre_tlwh[2] * pre_tlwh[3] > opt.min_box_area and not vertical:
                ...
                fps = skip_frames / (time.time() - start_time)
        else:
            online_im = plot_tracking(im0s, online_tlwhs, previous_tlwhs, online_ids, line, fps_pro=fps_process, fps=fps)
    ...
    LOGGER.info(f'Speed: %.1fms pre-process, %.1fms detector, %.1fms NMS, %.1fms tracker, %.1fms store-var per image at shape {(1, 3, *imgsz)}' % t)
    LOGGER.info(f'Speed detect person: %.5fs min time, %.5fs max_time, %.5fs avg_time' % (min_time, max_time, sumtime/seen))
