...
def plot_tracking(image, tlwhs, previous_tlwhs, obj_ids, line, scores=None,  fps_pro=0, fps=0., ids2=None):
    ...
    cv2.putText(im, 'FPS: %.2f - avgFPS: %.2f - num: %d' % (fps_pro, fps, len(tlwhs)),
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        # print('{0:<50} {1:<50} '.format("Current Point: "+str(cur_center), "Previous: "+str(pre_center)))
        ...
        if intersect(cur_center, pre_center, line[0], line[1]):
            if horizontal_True_vertical_False:
                if cur_center[1] < pre_center[1]:  
                    counter['up'] += 1  
                    cv2.line(im, line[0], line[1], (255, 255, 0), thickness=2, lineType=cv2.LINE_AA)
